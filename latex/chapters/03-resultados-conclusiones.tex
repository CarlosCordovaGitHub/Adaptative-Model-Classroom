% Apartado de Resultados con decimales reemplazados por puntos y coherencia revisada
...
\begin{itemize}
    \item \textbf{Total de perfiles de estudiantes virtuales:} 10 perfiles con parámetros psicométricos controlados, distribuidos uniformemente en el rango de habilidad $\theta \in [-2.0, +2.0]$.
    \item \textbf{Sesiones de evaluación completadas:} 28 sesiones simuladas correspondientes a los distintos escenarios de validación.
    \item \textbf{Interacciones ítem-estudiante registradas:} aproximadamente 180 interacciones completas, cada una con registro detallado de la respuesta del estudiante, estimación de habilidad, probabilidad de dominio por skill y recomendación generada.
    \item \textbf{Archivos de auditoría generados:} 180 archivos JSON con timestamps únicos, almacenados en estructura jerárquica \textit{runtime/logs/\{student\_id\}/\{session\_id\}/}, garantizando trazabilidad completa de todas las decisiones algorítmicas.
    \item \textbf{Tamaño total de datos persistidos:} aproximadamente 52 MB de información estructurada, incluyendo estados de estudiantes, logs de auditoría y métricas de sesión.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Parámetro de discriminación (a):} rango [0.5, 2.5], distribución log-normal con $\mu = 0.3$, $\sigma = 0.4$.
    \item \textbf{Parámetro de dificultad (b):} rango [-3.0, +3.0], distribución uniforme con cobertura balanceada (33\% fácil, 33\% medio, 33\% difícil).
    \item \textbf{Parámetro de adivinanza (c):} rango [0.0, 0.25], distribución uniforme.
    \item \textbf{Distribución por habilidad:} 100 ítems para ''regla de la potencia'', 100 ítems para ''regla de la cadena''.
\end{itemize}
...
\textbf{Verificación de Criterios de Validación}
...
La Tabla~\ref{tab:verificacion-criterios} resume los resultados obtenidos en la verificación de los criterios de validación técnica del sistema adaptativo.

\begin{table}[H]
\centering
\caption{Verificación de criterios de validación técnica}
\label{tab:verificacion-criterios}
\renewcommand{\arraystretch}{1.15}
\begin{spacing}{1}
\small
\begin{tabular}{|p{1.2cm}|p{3.0cm}|p{2.6cm}|p{1.8cm}|p{2.6cm}|p{1.8cm}|}
\hline
\textbf{Crit.} & \textbf{Hipótesis} & \textbf{Métrica} & \textbf{Umbral} & \textbf{Observado} & \textbf{Estado} \\
\hline
H1 & Precisión diagnóstica & RMSE($\theta$) & $< 0.65$ & 0.479 & \cellcolor{verdozo}\textbf{OK} \\
\hline
H2 & Reducción de incertidumbre & SE($\theta$) & $\leq 0.40$ & 0.38 & \cellcolor{verdozo}\textbf{OK} \\
\hline
H3 & Eficiencia adaptativa & N ítems & $\leq 15$ & 6 & \cellcolor{verdozo}\textbf{OK} \\
\hline
H4 & Equidad diagnóstica & CV(RMSE) & $< 40\%$ & 34.2\% & \cellcolor{verdozo}\textbf{OK} \\
\hline
H5 & Calidad predictiva & Brier & $< 0.30$ & 0.077 & \cellcolor{verdozo}\textbf{OK} \\
\hline
H6 & Rendimiento & Latencia P95 & $< 500$ ms & $\sim$450 ms & \cellcolor{verdozo}\textbf{OK} \\
\hline
H7 & Estabilidad & Tasa error & $< 1\%$ & 0.0\% & \cellcolor{verdozo}\textbf{OK} \\
\hline
\end{tabular}
\end{spacing}
\end{table}

\textbf{Resultado global:} El sistema cumplió 7/7 criterios de validación (100\%), superando los umbrales de aceptación definidos en todos los aspectos evaluados.\\

\subsection{Precisión Diagnóstica}
\begin{enumerate}

\item\textbf{Test de Convergencia de $\theta$}
...
\begin{itemize}
    \item \textbf{Error cuadrático medio (RMSE):} $0.479$
    \item \textbf{Error absoluto medio (MAE):} $0.479$
    \item \textbf{Error máximo observado:} $1.336$ (perfil $\theta = +2.0$)
    \item \textbf{Porcentaje de estudiantes con error $< 0.8$:} $66.7\%$ (6/9)
\end{itemize}
...
\item \textbf{Test de Reducción de Incertidumbre}
...
\begin{itemize}
    \item \textbf{SE($\theta$) inicial:} 0.9305 (prior $N(0.1)$)
    \item \textbf{SE($\theta$) final:} 0.6744
    \item \textbf{Reducción absoluta:} 0.2562 (27.5\% de reducción)
    \item \textbf{Monotonicidad:} 100.0\% (el SE($\theta$) disminuyó en cada iteración sin incrementos)
\end{itemize}

\textbf{Análisis:} el sistema demostró una reducción monotónica perfecta del error estándar, cumpliendo el criterio de estabilidad algorítmica.\\

La Figura~\ref{fig:evolucion-se} documenta gráficamente este comportamiento, mostrando una curva descendente continua que parte desde un valor inicial elevado (asociado con la distribución prior) y converge progresivamente hacia niveles de precisión diagnóstica superiores con cada ítem administrado. La línea horizontal discontinua en rojo marca el umbral objetivo de SE $\leq 0.40$, mientras que la zona sombreada en verde delimita la región de precisión aceptable. El recuadro informativo en la esquina superior izquierda cuantifica la magnitud de la reducción lograda, evidenciando una disminución del $49.9\%$ respecto al valor inicial.
...
\item \textbf{Eficiencia Adaptativa}
...
La Tabla~\ref{tab:items-convergencia-normal} presenta los Ítems requeridos para Convergencia (SE $\leq 0.40$).
...
\item \textbf{Equidad Diagnóstica}
\vspace{0.5cm}
...
\begin{itemize}
    \item \textbf{Grupo bajo:} $\theta \in [-2.0, -0.67)$ (n=3)
    \item \textbf{Grupo medio:} $\theta \in [-0.67, +0.67]$ (n=3)
    \item \textbf{Grupo alto:} $\theta \in (+0.67, +2.0]$ (n=3)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Coeficiente de variación (CV):} $34.2\%$
    \item \textbf{RMSE mínimo:} $0.378$ (grupo bajo)
    \item \textbf{RMSE máximo:} $0.507$ (grupo alto)
    \item \textbf{Diferencia relativa:} $34.2\%$
\end{itemize}

\textbf{Análisis:} El sistema cumplió el criterio de equidad diagnóstica ($CV < 40\%$), alcanzando un coeficiente de variación de $34.2\%$. Además, todos los grupos presentaron $RMSE < 0.70$, cumpliendo el criterio de suficiencia que establece que ningún grupo debe experimentar errores excesivos.\\
...
\begin{enumerate}
\item \textbf{Test de Stopping Rules}
...
\textbf{Razón de parada:} \texttt{ALL\_SKILLS\_MASTERED} \; (min = 0.947,\; items = 5)
\vspace{0.5cm}

\textbf{Interpretación}: El sistema detectó que el estudiante alcanzó dominio completo (p\_mastery $\geq 0.85$) en ambas habilidades evaluadas tras administrar 5 ítems, con una probabilidad mínima de dominio de 0.947.\\

\textbf{Análisis}: El mecanismo de parada funcionó correctamente, identificando tempranamente la situación de dominio completo y evitando la administración innecesaria de ítems adicionales. Este comportamiento es deseable en sistemas adaptativos, ya que optimiza el tiempo de evaluación sin comprometer la precisión diagnóstica.\\

\item \textbf{Test de Replay Determinista}
\vspace{0.5cm}
...
\textbf{Resultados:}
\begin{itemize}
\item \textbf{$\hat{\theta}$ en sesión original}: 1.2815  

\item \textbf{$\hat{\theta}$ en sesión replay}: 1.2815  

\item \textbf{Diferencia absoluta}: 0.0000 ($< 1\times10^{-6}$)
\end{itemize}
\vspace{0.5cm}

\textbf{Análisis}: El sistema demostró determinismo perfecto, reproduciendo exactamente el mismo estado final al procesar los mismos eventos en el mismo orden. Esta característica es fundamental para:
\begin{itemize}
    \item\textbf{ Auditoría y trazabilidad:} permite verificar decisiones algorítmicas en sesiones pasadas.
    \item\textbf{ Debugging:} facilita la identificación de errores mediante reproducción exacta de escenarios problemáticos.
    \item\textbf{ Validación científica:} garantiza la reproducibilidad de experimentos, requisito esencial en investigaciones de ingeniería de software.
\end{itemize}
...
\item \textbf{Test de Estrés bajo Concurrencia}
...
\begin{itemize}
    \item\textbf{ Usuarios concurrentes:} 50 usuarios simulados
    \item\textbf{ Tasa de spawn:} 5 usuarios/segundo
    \item\textbf{ Duración:} 5 minutos de carga sostenida
    \item\textbf{ Operaciones:} envío de respuestas (/b/events), consulta de métricas (/metrics), health checks
\end{itemize}
\vspace{0.5cm}
...
\hline
\textbf{Métrica} & \textbf{Valor Observado} & \textbf{Umbral} & \textbf{Estado} \\
\hline
...
\item \textbf{Validación del Decaimiento Temporal (Curva del Olvido)}
\vspace{0.5cm}
...
\begin{itemize}
    \item\textbf{ Estudiante simulado:} perfil con $\theta = 0.5$, mastery inicial bajo
    \item\textbf{ Fase 1:} sesión de aprendizaje intensiva (25 ítems) hasta alcanzar dominio completo
    \item\textbf{ Intervalo temporal simulado:} 7 días sin interacción con el sistema
    \item\textbf{ Fase 2:} nueva sesión de evaluación para medir el efecto del decay
\end{itemize}
\vspace{0.5cm}
...
\begin{itemize}
    \item \textbf{Precisión superior:} errores de estimación significativamente inferiores a los umbrales establecidos.
    \item \textbf{Eficiencia excepcional:} reducción del $60$-$70\%$ en número de ítems respecto a evaluaciones tradicionales.
    \item \textbf{Equidad garantizada:} precisión comparable entre estudiantes de distinto nivel.
    \item \textbf{Calidad predictiva excelente:} calibración de probabilidades muy superior al baseline aleatorio.
    \item \textbf{Viabilidad operativa:} escalabilidad y estabilidad demostradas bajo condiciones reales de uso.
    \item \textbf{Innovación técnica:} implementación exitosa de decay temporal para modelar olvido.
\end{itemize}
...
\item La validación técnica del Sistema de Evaluación Adaptativa a partir de simulación computacional evidencia la viabilidad técnica, algorítmica y operativa del modelo híbrido que propone integrar la Teoría de Respuesta al Ítem (IRT 3PL) con técnicas bayesianas de rastreado del conocimiento (BKT). El cumplimiento integral de los siete criterios de validación establecidos a priori ($100\%$ del criterio de aceptación) constituye una sólida evidencia empírica de que el sistema logra alta precisión diagnóstica (RMSE = $0.479$, $26\%$ por debajo de la línea de corte) y una calidad predictiva excelente (Brier Score = $0.077$).

\item Los resultados obtenidos muestran una eficiencia notable del sistema adaptativo, evidenciada por la convergencia en un promedio de 6.0 ítems, lo que representa una reducción entre el $60\%$ y el $70\%$ frente a las evaluaciones tradicionales. Este comportamiento confirma la efectividad de la política de selección orientada a maximizar la información y reducir rápidamente el error estándar de medición.

\item La integración del rastreo bayesiano del conocimiento permitió mantener y actualizar una probabilidad de dominio por habilidad, logrando una equidad diagnóstica entre perfiles heterogéneos de estudiantes, con un coeficiente de variación del RMSE del $34.2\%$. Este resultado evidencia que el sistema no favorece ni penaliza sistemáticamente a grupos específicos de habilidad.

\item La implementación del Componente B como un bucle MAPE-K permitió garantizar trazabilidad, explicabilidad y coherencia operativa en el proceso de evaluación adaptativa. La viabilidad operativa del sistema como servicio software quedó demostrada mediante una latencia P95 inferior a 500 ms bajo una carga de 50 usuarios concurrentes y una tasa de error del $0.0\%$.

\item El diseño experimental riguroso fundamentado en el método hipotético-deductivo y validado mediante técnicas de simulación estocástica Monte Carlo permitió verificar de forma objetiva el cumplimiento de los criterios de validación establecidos, aportando una base empírica sólida que respalda la correcta integración entre componentes y la trazabilidad del proceso de evaluación.

\item La implementación satisfactoria del mecanismo de decaimiento temporal del conocimiento representa una aportación técnica distintiva que permite modelar fenómenos de olvido en situaciones educativas donde los estudiantes retoman actividades tras largos periodos de inactividad. En conjunto, estos resultados permiten avanzar hacia fases posteriores de validación ecológica con estudiantes reales, necesarias para evaluar el impacto pedagógico efectivo del sistema y factores cualitativos que trascienden el comportamiento algorítmico.
...
\begin{itemize}
    \item \textbf{Validación con Estudiantes Reales}\\
    ...
    \item \textbf{Calibración del Banco de Ítems}\\
    Los parámetros IRT actuales son sintéticos. Para un despliegue real, calibrar los parámetros (a, b, c) con datos empíricos usando software especializado como R (paquete mirt) o Python (py-irt). Implementar calibración en línea para que los parámetros se ajusten de forma continua a medida que el sistema va acumulando respuestas reales.

    \item \textbf{Mejoras de Escalabilidad}\\
    ...
    \item \textbf{Explicabilidad y Equidad}\\
    ...
    \item \textbf{Diseminación del Conocimiento}\\   
